...
from reckon_copilot.agents.analytics import FrappeAnalyticsDataSource
from reckon_copilot.agents.forecasting import ForecastingError, run_forecasting
...
def run(
    ...
    try:
        ...
        return {"ok": False, "access_denied": True, "agent": "forecasting", "message": str(error)}
    except ForecastingError as error:
        return {"ok": False, "agent": "forecasting", "message": str(error)}
    except Exception:
        return {"ok": False, "agent": "forecasting", "message": "Forecasting could not complete for this page. No ERP data was changed."}
